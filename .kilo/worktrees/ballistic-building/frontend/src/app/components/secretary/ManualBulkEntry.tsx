...
import { firebaseFeeStructureService, firebaseSilambanFeeService, FeeStructure, SilambanFeeStructure, firebaseSchoolFeeRequestService } from '../../services/firebaseData';
...
export default function ManualBulkEntry({ onStudentsGenerated, selectedProgram, availableSchools }: ManualBulkEntryProps) {
  ...
  const [acceptedFeeRequests, setAcceptedFeeRequests] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      try {
        ...
        const [kFees, sFees, feeReqs] = await Promise.all([
          firebaseFeeStructureService.getActive(),
          firebaseSilambanFeeService.getActive(),
          user ? firebaseSchoolFeeRequestService.getBySecretary(user.uid) : Promise.resolve([])
        ]);
        setKarateFees(kFees);
        setSilambamFees(sFees);
        setAcceptedFeeRequests(feeReqs.filter((r: any) => r.status === 'approved'));
      } catch (err) {
        console.error(err);
  ...
  const handleGenerate = () => {
    ...
    const formattedStudents = studentsData.map((s, i) => {
      ...
      const schoolReq = acceptedFeeRequests.find((r: any) =>
        r.schoolId === s.schoolId &&
        (String(r.programType).toLowerCase() === selectedProgram.toLowerCase() || (String(r.programType).toLowerCase() === 'silambam' && selectedProgram.toLowerCase() === 'selambam'))
      );
      ...
      if (selectedProgram === 'KARATE') {
        ...
        if (matchingFee) {
          ...
          if (schoolReq) {
             const requestedFeeObj = schoolReq.requestedFees?.find((f: any) => String(f.name).toLowerCase() === String(beltLevel).toLowerCase());
             if (requestedFeeObj && !isNaN(Number(requestedFeeObj.fee))) {
                fee = Number(requestedFeeObj.fee);
             } else {
        ...
        if (schoolReq) {
           const requestedFeeObj = schoolReq.requestedFees?.find((f: any) => String(f.name).toLowerCase() === stageLabel.toLowerCase());
           if (requestedFeeObj && !isNaN(Number(requestedFeeObj.fee))) {
              fee = Number(requestedFeeObj.fee);
           } else {
              ...
              <div className="col-span-2 md:col-span-2">
                ...
                {selectedProgram === 'KARATE' ? (
                  ...
                  >
                    ...
                    {beltOptions.map(b => {
                        let rowFee = b.fee;
                        const req = acceptedFeeRequests.find((r: any) => r.schoolId === s.schoolId && String(r.programType).toLowerCase() === selectedProgram.toLowerCase());
                        if (req) {
                          const found = req.requestedFees?.find((f: any) => String(f.name).toLowerCase() === String(b.label).toLowerCase());
                          if (found && !isNaN(Number(found.fee))) rowFee = Number(found.fee);
                        }
                        return <option key={b.value} value={b.value}>{b.label} — ₹{rowFee}</option>;
                    })}
                  </select>
                ) : (
                  <select 
                  ...
                  >
                    ...
                    {stageOptions.map(st => {
                        let rowFee = st.fee;
                        const req = acceptedFeeRequests.find((r: any) => r.schoolId === s.schoolId && (String(r.programType).toLowerCase() === 'silambam' || String(r.programType).toLowerCase() === 'selambam'));
                        if (req) {
                          const searchName = `stage ${st.value}`;
                          const found = req.requestedFees?.find((f: any) => String(f.name).toLowerCase() === searchName);
                          if (found && !isNaN(Number(found.fee))) rowFee = Number(found.fee);
                        }
                        return <option key={st.value} value={st.value}>{st.label} — ₹{rowFee}</option>;
                    })}
                  </select>
                )}
              </div>
