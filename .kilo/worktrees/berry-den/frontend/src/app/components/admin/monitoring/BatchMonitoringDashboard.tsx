...
import { 
  ...
  firebaseSecretaryService,
  firebaseSecretarySchoolService
} from "../../../services/firebaseData";
import { Batch, BeltTest, School, Referee, StudentRecord, Secretary } from "../../../types/admin";
...
export default function BatchMonitoringDashboard() {
  ...
  const [secretaryLinks, setSecretaryLinks] = useState<{secretaryId: string, schoolId: string}[]>([]);
  ...
  const fetchData = async (showLoading = true) => {
    ...
    try {
      ...
      const [
        bData, tData, sData, rData, secData, stuData, linkData
      ] = await Promise.all([
        ...
        firebaseSecretaryService.getAll(),
        firebaseStudentService.getAll(programFilter),
        firebaseSecretarySchoolService.getAllLinks()
      ]);
  ...
  const {
    ...
    const enriched = batches.map(batch => {
      ...
      const batchReferees = referees.filter(r => batch.refereeIds?.includes(r.id));
      ...
      let secretaryName = "Unknown";
      if (school) {
        // Try to find if this school is linked to a secretary in secretarySchools
        const link = secretaryLinks.find(l => l.schoolId === school.id);
        if (link) {
          const sec = secretaries.find(s => s.uid === link.secretaryId);
          if (sec) secretaryName = sec.fullName;
        } else if (school.createdBy) {
          // Fallback to createdBy if link not found (some older records might only have this)
          const sec = secretaries.find(s => s.uid === school.createdBy);
          if (sec) secretaryName = sec.fullName;
        }
      }

      // Fallback: If still unknown (e.g., individual batches or unlinked schools), 
      // check if the students in the batch belong to a secretary
      if (secretaryName === "Unknown" && batchStudents.length > 0) {
        const studentWithSec = batchStudents.find(s => !!s.secretaryId);
        if (studentWithSec) {
          const sec = secretaries.find(s => s.uid === studentWithSec.secretaryId);
          if (sec) secretaryName = sec.fullName;
        }
      }
      ...
      return {
        ...
        secretaryName,
        refereeNames: batchReferees.map(r => r.name).join(", ") || "Unassigned",
    ...
    // Secretary stats
    const secMap: Record<string, { total: number, inProgress: number, completed: number, pending: number }> = {};
    enriched.forEach(b => {
      if (b.secretaryName !== "Unknown") {
        if (!secMap[b.secretaryName]) secMap[b.secretaryName] = { total: 0, inProgress: 0, completed: 0, pending: 0 };
        secMap[b.secretaryName].total++;
        if (b.status === 'ongoing') secMap[b.secretaryName].inProgress++;
        else if (b.status === 'completed') secMap[b.secretaryName].completed++;
        else secMap[b.secretaryName].pending++;
      }
    });
    ...
    return {
      ...
      secretaryStats: Object.entries(secMap).map(([name, stats]) => ({ name, ...stats }))
    };
  }, [batches, tests, schools, referees, secretaries, students, secretaryLinks]);
  ...
  const filteredBatches = enrichedBatches.filter(b => {
    ...
    if (searchQuery) {
      ...
      return (
        ...
        b.secretaryName.toLowerCase().includes(q) ||
        b.refereeNames.toLowerCase().includes(q)
      );
    }
    return true;
  });
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden flex flex-col h-full min-h-[500px]">
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 flex flex-col sm:flex-row gap-4 justify-between">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search batches, schools, secretaries..."
                  value={searchQuery}
                  ...
                  {filteredBatches.length === 0 ? (
                    ...
                    filteredBatches.map(batch => (
                      <tr key={batch.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="font-medium text-zinc-700 dark:text-zinc-300 max-w-[150px] truncate" title={batch.refereeNames}>
                            {batch.refereeNames}
                          </div>
                        </td>
                ...
                liveBatches.slice(0, 3).map(batch => (
                  <div key={batch.id} className="bg-white/10 rounded-xl p-3 backdrop-blur-sm cursor-pointer hover:bg-white/20 transition" onClick={() => setSelectedBatch(batch)}>
                    ...
                    <div className="text-xs text-indigo-100 space-y-1">
                      <div className="flex justify-between"><span>Examiner:</span> <span className="font-medium text-white">{batch.refereeNames.split(',')[0]}</span></div>
      ...
      <div className="relative bg-white dark:bg-zinc-950 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-zinc-200 dark:border-zinc-800 animate-fade-in">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <DetailItem icon={<Activity />} label="Examiner" value={batch.refereeNames || "Not Assigned"} />
          </div>
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                  {batch.students.map((student: any, idx: number) => (
                    <tr key={student.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                      ...
                      <td className="px-4 py-2.5 text-center">
                        {student.score !== undefined ? (
                          <span className={`font-bold text-sm ${student.result === 'fail' ? 'text-red-600' : 'text-green-600'}`}>
