...
import { firebaseSecretaryService } from "../../services/firebaseData";
...
export default function SecretaryRegister() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (authConfig && !authConfig.roles.secretary.registration.enabled) {
      showToast(authConfig.roles.secretary.registration.message || "Secretary registration is closed.", "error");
      return;
    }
    ...
    try {
      ...
      // 2. Directly create secretary profile in Firestore — no approval needed
      await firebaseSecretaryService.create({
  ...
  return (
    <AuthLayout
      ...
      subtitle="Shadow Kai Karate - Secretary Portal"
    >

        {authConfig && !authConfig.roles.secretary.registration.enabled ? (
          <div className="text-center p-8 bg-zinc-950/50 rounded-2xl border border-red-500/30 backdrop-blur-sm animate-fade-in">
            ...
            <p className="text-zinc-400 text-sm">{authConfig.roles.secretary.registration.message || "Secretary registration is temporarily unavailable. Please contact the administrator."}</p>
        ...
        <form onSubmit={handleSignUp} className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <p className="text-center md:col-span-2 text-sm text-zinc-400 mt-2">
            ...
            <button type="button" onClick={() => navigate("/secretary/login")} className="text-blue-500 hover:text-blue-400 font-semibold transition-colors">
              Sign In
            </button>
          </p>
        </form>
