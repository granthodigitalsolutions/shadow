...
import { firebaseAdminAuthService, firebaseSecretaryAuthService } from "../../services/firebaseData";

interface ProtectedRouteProps {
  children: React.ReactNode;
}

export default function SecretaryProtectedRoute({ children }: ProtectedRouteProps) {
  const [loading, setLoading] = useState(true);
  const [status, setStatus] = useState<"admin" | "secretary" | "unauthorized" | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  useEffect(() => {
    const unsubscribe = firebaseAuthService.onAuthStateChange(async (user) => {
//       console.log("SecretaryProtectedRoute: auth state changed", user?.uid);
      if (user) {
        try {
...
//           console.log("isAdmin result:", isAdmin);
          if (isAdmin) {
            ...
            const timeoutPromise = new Promise<boolean>((_, reject) => 
              setTimeout(() => reject(new Error("Timeout waiting for secretary authorization")), 8000)
            );
            
            const isSecretary = await Promise.race([
              firebaseSecretaryAuthService.isSecretary(user.uid),
              timeoutPromise
            ]);
            
//             console.log("isSecretary result:", isSecretary);
            if (isSecretary) {
              setStatus("secretary");
            } else {
          ...
          console.error("Error in SecretaryProtectedRoute auth check:", error);
  ...
  // Admins can access secretary portal too, if they want
  if (status === "secretary" || status === "admin") {
    return <>{children}</>;
  }
