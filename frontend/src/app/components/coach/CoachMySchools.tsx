...
import { School as SchoolIcon, Plus, RefreshCw, Users, CheckCircle2, X } from "lucide-react";
...
export default function CoachMySchools() {
  const { mySchools, selectedSchoolId, setSelectedSchoolId, refreshMySchools, loading } = useCoachSchool();
  const { showToast } = useToast();
  const navigate = useNavigate();

  const [stats, setStats] = useState<Record<string, { students: number }>>({});
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      try {
        const allMyStudents = await firebaseStudentService.getByCoach(user.uid);
        ...
        for (const school of mySchools) {
          ...
          result[school.id] = { students: count };
        }
        setStats(result);
      } catch (err) {
  ...
  const handleLinkExisting = async () => {
    ...
    setSaving(true);
    try {
      await firebaseCoachSchoolService.linkSchool(user.uid, school.id, school.name);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
              Select a school to manage its students, or link another one.
            </p>
          </div>
