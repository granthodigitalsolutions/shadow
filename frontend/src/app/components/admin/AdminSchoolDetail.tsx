...
export default function AdminSchoolDetail() {
  const { coachId, schoolId, program } = useParams<{ coachId: string; schoolId: string; program: string }>();
  ...
  // Coach accounts and schools are shared across programs, but this page is
  ...
  const loadData = async () => {
    if (!coachId || !schoolId) return;
    setLoading(true);
    try {
      const [schoolData, coachData, allCoachStudents] = await Promise.all([
        firebaseSchoolService.getById(schoolId),
        firebaseCoachAuthService.getCoachByUid(coachId),
        firebaseStudentService.getByCoach(coachId),
      ]);
      setSchool(schoolData);
      setCoach(coachData);
      setStudents(
        allCoachStudents.filter(
  ...
  return (
    <AdminLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <button
          onClick={() => navigate(`/admin/${program}/coaches`)}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 flex items-center gap-1.5 mt-1">
                  <User className="w-3.5 h-3.5" /> Managed by {coach?.fullName || coach?.email || "Coach"}
                </p>
              </div>
            </div>
            <button
