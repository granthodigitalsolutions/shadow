          ...
          <div className="mt-8 bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm animate-fade-in">
            ...
            <div className="grid grid-cols-1 max-w-sm gap-6">
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Coach Access</h4>
                <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900 rounded-lg">
                  ...
                  <span className={`flex items-center gap-1 text-xs font-bold ${authConfig.roles?.coach?.registration?.enabled ? 'text-green-600' : 'text-red-600'}`}>
                    {authConfig.roles?.coach?.registration?.enabled ? <><ShieldCheck className="w-4 h-4"/> OPEN</> : <><ShieldAlert className="w-4 h-4"/> CLOSED</>}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900 rounded-lg">
                  <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Login</span>
                  <span className={`flex items-center gap-1 text-xs font-bold ${authConfig.roles?.coach?.login?.enabled ? 'text-green-600' : 'text-red-600'}`}>
                    {authConfig.roles?.coach?.login?.enabled ? <><ShieldCheck className="w-4 h-4"/> OPEN</> : <><ShieldAlert className="w-4 h-4"/> CLOSED</>}
                  </span>
                </div>
              </div>
            </div>
