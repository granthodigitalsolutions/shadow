...
import { Coach } from "../../types/admin";
import { useProgram } from "../../contexts/ProgramContext";
import AdminLayout from "./AdminLayout";

type CoachSummary = Awaited<ReturnType<typeof firebaseCoachSchoolService.getCoachSummary>>;

const EMPTY_SUMMARY: CoachSummary = {
...
export default function CoachManagement() {
  ...
  // to coaches who actually have students in that state.
  ...
  // Per-coach school/student/fee aggregates, keyed by uid
  const [summaries, setSummaries] = useState<Record<string, CoachSummary>>({});
  const [summariesLoading, setSummariesLoading] = useState(false);

  // Schools drill-down
  const [viewingSchoolsFor, setViewingSchoolsFor] = useState<Coach | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Coach accounts are shared across both programs (a Coach can manage
      // Karate schools and Silambam schools alike) — fetch every coach regardless
      // of which program tab is active; only the schools/students summary below is
      // scoped per program.
      const data = await firebaseCoachService.getAll();
      setCoaches(data);
      loadSummaries(data);
    } catch (err) {
      console.error("Error fetching coaches data:", err);
      showToast("Failed to load coach data", "error");
    } finally {
      setLoading(false);
    }
  };

  const loadSummaries = async (list: Coach[]) => {
    setSummariesLoading(true);
    try {
      const entries = await Promise.all(
        list.map(async (coach) => [coach.uid, await firebaseCoachSchoolService.getCoachSummary(coach.uid, currentProgram)] as const),
      );
      setSummaries(Object.fromEntries(entries));
    } catch (err) {
      console.error("Failed to load coach summaries:", err);
    } finally {
  ...
  const handleToggleActive = async (coach: Coach) => {
    const actionStr = coach.active ? "disable" : "enable";
    const confirmed = await showConfirm({
      title: `${actionStr.charAt(0).toUpperCase() + actionStr.slice(1)} Coach?`,
      message: `Are you sure you want to ${actionStr} access for ${coach.fullName}?`,
      confirmText: `Yes, ${actionStr}`,
      variant: coach.active ? "warning" : "success",
    });

    if (!confirmed) return;

    try {
      await firebaseCoachService.toggleActive(coach.uid, !coach.active);
      showToast(`Coach account ${actionStr}d`, "success");
      fetchData();
    } catch (err) {
  ...
  const saveEdit = async () => {
    ...
    setSaving(true);
    try {
      await firebaseCoachService.update(editCoach.uid, editForm);
  ...
  const handleDelete = async (coach: Coach) => {
    const confirmed = await showConfirm({
      title: "Delete Coach?",
      message: `Are you sure you want to permanently delete ${coach.fullName}? This cannot be undone.`,
    ...
    try {
      await firebaseCoachService.delete(coach.uid);
  ...
  // Rolled-up totals across every coach (all schools, all students)
  ...
  const filteredCoaches = coaches.filter(coach => {
    const matchesSearch = (coach.fullName || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
           ...
           (coach.academy || "").toLowerCase().includes(searchQuery.toLowerCase());

    if (statusFilter === "active" && !coach.active) return false;
    if (statusFilter === "disabled" && coach.active) return false;

    if (activityFilter === "pending" && !((summaries[coach.uid]?.pendingStudents ?? 0) > 0)) return false;
    if (activityFilter === "confirmed" && !((summaries[coach.uid]?.confirmedStudents ?? 0) > 0)) return false;
        ...
        <div className="bg-white dark:bg-zinc-950 rounded-lg shadow-md dark:shadow-none dark:border dark:border-zinc-800 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mt-0.5">Manage academy coaches and the schools they run</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 text-center">
              <p className="text-3xl font-bold text-zinc-900 dark:text-zinc-50 tracking-tight">{coaches.length}</p>
              <p className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mt-1">Total Coaches</p>
            </div>
            <button
              type="button"
              ...
              title="Show only coaches with pending students"
            >
            ...
            <button
              ...
              title="Show only coaches with confirmed students"
            >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCoaches.map(coach => (
              <div
                key={coach.uid}
                className={`relative bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300 p-5 group flex flex-col h-full ${!coach.active ? 'opacity-75 grayscale-[20%]' : ''}`}
              >
                {/* Top Section */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    {/* Avatar */}
                    <div className={`w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 text-base font-bold shadow-sm ${coach.active ? 'bg-gradient-to-br from-blue-400 to-blue-500 text-blue-950' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400'}`}>
                ...
                <div className="flex flex-col gap-2.5 mt-1 mb-4 text-[13px] text-zinc-600 dark:text-zinc-400">
                  ...
                  <div className="flex justify-between items-center text-[12px] text-zinc-500">
                    ...
                    <span className="font-medium text-zinc-700 dark:text-zinc-300">{formatSafeDate(coach.createdAt?.toDate?.() || coach.createdAt)}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 flex-grow content-start">
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl p-2.5 text-center">
                    <p className="text-lg font-bold text-zinc-900 dark:text-zinc-50">{summariesLoading ? "…" : summaries[coach.uid]?.totalSchools ?? 0}</p>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Schools</p>
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl p-2.5 text-center">
                    <p className="text-lg font-bold text-zinc-900 dark:text-zinc-50">{summariesLoading ? "…" : summaries[coach.uid]?.totalStudents ?? 0}</p>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Students</p>
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl p-2.5 text-center">
                    <p className="text-lg font-bold text-blue-500">{summariesLoading ? "…" : summaries[coach.uid]?.pendingStudents ?? 0}</p>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Pending</p>
                  </div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl p-2.5 text-center">
                    <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{summariesLoading ? "…" : summaries[coach.uid]?.confirmedStudents ?? 0}</p>
                  ...
                  <div className="col-span-2 bg-zinc-50 dark:bg-zinc-900 rounded-xl p-2.5 text-center">
                    <p className="text-lg font-bold text-zinc-900 dark:text-zinc-50">₹{(summariesLoading ? 0 : summaries[coach.uid]?.totalFee ?? 0).toLocaleString()}</p>
                ...
                {/* View Schools */}
                <button
                  onClick={() => setViewingSchoolsFor(coach)}
                ...
                <div className="flex items-center justify-end gap-1.5 pt-3 border-t border-zinc-100 dark:border-zinc-800/80 mt-auto">
                  <button
                    ...
                    title={coach.active ? "Disable Account" : "Enable Account"}
                    className={`p-2 rounded-lg transition-colors ${coach.active ? 'text-zinc-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20' : 'text-emerald-600 hover:bg-emerald-50 dark:text-emerald-500 dark:hover:bg-emerald-950/30'}`}
                  >
                    {coach.active ? <XCircle className="w-4 h-4" /> : <Check className="w-4 h-4" />}
                  </button>
                  <button
      ...
      {viewingSchoolsFor && (() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-950 rounded-2xl shadow-2xl w-full max-w-3xl border border-zinc-200 dark:border-zinc-800 flex flex-col max-h-[90vh]">
            ...
            {/* Summary Bar */}
            <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 grid grid-cols-3 sm:grid-cols-5 gap-3">
            ...
            <div className="p-6 overflow-y-auto">
              {summariesLoading ? (
                ...
                <div className="py-12 text-center text-zinc-400">This coach is not managing any schools yet.</div>
              ) : (
                <>
                        ...
                        <tr key={school.id}>
                          ...
                          <td className="py-3 pl-3 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <Link
                                to={`/admin/${program}/coaches/${viewingSchoolsFor.uid}/schools/${school.id}`}
                                onClick={() => setViewingSchoolsFor(null)}
