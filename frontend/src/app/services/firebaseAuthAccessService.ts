...
export interface AuthAccessConfig {
  roles: {
    coach: AuthAccessRoleSettings;
    [key: string]: AuthAccessRoleSettings; // For future-proofing
  };
...
export const firebaseAuthAccessService = {
  ...
  subscribe: (onUpdate?: (config: AuthAccessConfig) => void) => {
    ...
    firebaseAuthAccessService._unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        const raw = docSnap.data() as AuthAccessConfig;
        // Deep-merge with defaults so any missing role keys never cause downstream crashes
        const data: AuthAccessConfig = {
          ...DEFAULT_CONFIG,
          ...raw,
          roles: {
            ...
            ...raw.roles,
            coach: {
              ...DEFAULT_CONFIG.roles.coach,
              ...(raw.roles?.coach ?? {}),
              registration: {
                ...DEFAULT_CONFIG.roles.coach.registration,
                ...(raw.roles?.coach?.registration ?? {}),
              },
              login: {
                ...DEFAULT_CONFIG.roles.coach.login,
                ...(raw.roles?.coach?.login ?? {}),
              },
            },
          },
        };
  ...
  getCoachRegistrationMessage: () => firebaseAuthAccessService._cachedConfig.roles?.coach?.registration?.message || "Registration is temporarily closed.",
  getCoachLoginMessage: () => firebaseAuthAccessService._cachedConfig.roles?.coach?.login?.message || "Login is temporarily disabled.",
